# app.py (UI polished, side-by-side layout with drag-and-drop image upload)
...
with col1:
    ...
    optin_text = st.text_area("Paste opt-in text or URL", height=100)
    optin_image = st.file_uploader("Or upload opt-in screenshot", type=["png", "jpg", "jpeg"], key="optin")

    if optin_image:
        st.image(optin_image, caption="Uploaded Opt-In Screenshot", use_column_width=True)
    if optin_text:
        issues, snippets = check_opt_in_compliance(optin_text)
...
with col2:
    ...
    privacy_text = st.text_area("Paste privacy policy text or URL", height=100)
    privacy_image = st.file_uploader("Or upload privacy policy screenshot", type=["png", "jpg", "jpeg"], key="privacy")

    if privacy_image:
        st.image(privacy_image, caption="Uploaded Privacy Policy Screenshot", use_column_width=True)
    if privacy_text:
        issues, snippets = check_privacy_compliance(privacy_text)
        st.markdown("### 📋 Compliance Check")
        if not issues:
